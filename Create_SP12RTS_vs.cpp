...
// the data generated by Create_SP12RTS_dvs.sh is named as "SP12RTS_dvs.txt";
...
int main(){
    ...
    float lat,lon,depth,dvs,vs;
    set<float> s_lat,s_lon,s_depth;
    string tmpstr;
    ifstream fpin("SP12RTS_dvs.txt");

    while (fpin >> depth >> lon >> lat >> dvs){
        vs=(1+dvs/100.0)*Dvs(depth);
        raw_data.push_back({depth,lat,lon,vs});
        s_lat.insert(lat);
        s_lon.insert(lon);
