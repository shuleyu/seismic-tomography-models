...
int main(){
    ...
    auto tomo=Tomography::Model("TX2000_dvs.nc");

    // write nc file.
    int retval,ncid;
    string outfile="TX2000_vs.nc";
    ...
    string unit_v="vs, km/s, converted form dvs and PREM, however, it might be meaningless, use with care";
    ...
    for (size_t i=0;i<tomo.GetDepths().size();++i) {
        for (size_t j=0;j<tomo.GetLatitudes().size();++j) {
            for (size_t k=0;k<tomo.GetLongitudes().size();++k) {
                data[index]=Dvs(tomo.GetDepths()[i])*(1-tomo.GetValues()[index]/100.0);
                ++index;
            }
        }
    }
