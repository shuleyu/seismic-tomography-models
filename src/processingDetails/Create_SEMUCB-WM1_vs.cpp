...
int main(){
    ...
    while (fpin >> depth >> lon >> lat >> dvs >> dxi >> vsv >> vsh){
        depth=6371-depth;
        raw_data.push_back(sqrt((2*vsv*vsv+vsh*vsh)/3));
        s_lat.insert(lat);
        s_lon.insert(lon);
    ...
    // write nc file.
    int retval,ncid;
    string outfile="SEMUCB-WM1_vs.nc";
    ...
    string unit_v="vs, (km/s), Voigt-average of vsv and vsh, vs = sqrt((vsh*vsh+2*vsv*vsv)/3)";
